...
public class PlayerScript : MonoBehaviourPun
{
    ...
    private void OnTriggerEnter2D(Collider2D col)
    {
        if (view.IsMine)
        {
            ...
            if (col.gameObject.CompareTag("searchJobPopup"))
            {
                col.gameObject.transform.Find("SearchJobsPopUp").gameObject.SetActive(true);
            }
            if (col.gameObject.CompareTag("MyJobPopup"))
            {
                col.gameObject.transform.Find("MyJobs").gameObject.SetActive(true);
            }
        }
    }
    ...
    private void OnTriggerExit2D(Collider2D col)
    {
        if (view.IsMine)
        {
            ...
            {
                DeactivateChat();
                view.RPC("CloseChatWindow", RpcTarget.Others, col.gameObject.GetComponent<PhotonView>().ViewID, view.ViewID);
            }
            if (col.gameObject.CompareTag("searchJobPopup"))
            {
                col.gameObject.transform.Find("SearchJobsPopUp").gameObject.SetActive(false);
            }
            if (col.gameObject.CompareTag("MyJobPopup"))
            {
                col.gameObject.transform.Find("MyJobs").gameObject.SetActive(false);
            }
        }
    }
    ...
    [PunRPC]
    public void CloseChatWindow(int targetPlayerViewID, int senderViewId)
    {
        if (senderViewId == view.ViewID)
        {
            GameObject targetPlayer = FindOtherPlayerForChat(targetPlayerViewID);
